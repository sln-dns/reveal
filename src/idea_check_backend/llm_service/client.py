...
class LLMServiceClient:
    ...
    def _build_stub_response(self, prompt: str) -> str:
        if "scene_01_intro" in prompt:
            return json.dumps(
                {
                    "intro_text": (
                        "Вы только входите в этот маршрут. "
                        "Пусть начало будет лёгким: просто поймайте, что отзывается первым."
                    ),
                    "questions": [
                    ...
                    "transition_text": (
                        "Первый вайб уже появился. "
                        "Теперь можно сделать ещё один шаг и понять, куда этот вечер тянет дальше."
                    ),
                }
            )
        return json.dumps(
            {
                "intro_text": (
                    "Маршрут уже начинает оживать. "
                    "Теперь интересно поймать его ритм чуть точнее."
                ),
                "questions": [
                ...
                "transition_text": (
                    "Контур уже виден. "
                    "Дальше можно подойти ближе и посмотреть, как этот вайб держится в движении."
                ),
            }
        )
    ...
    def _build_fallback_generation(self, payload: SceneGenerationPayload) -> SceneGeneration:
        ...
        intro_title = payload.scene_title or payload.scene_type.replace("_", " ")
        intro_text = (
            f"{intro_title} уже рядом. "
            f"Без правильных ответов и без лишнего напряжения: просто выбери, что тебе сейчас ближе."
        )
        transition_text = (
            "Нужный сигнал уже появился. "
            "Дальше можно пойти чуть глубже и посмотреть, что начнёт открываться между строк."
        )
